...
The Swiper MCP server provides 8 tools for interacting with Swiper documentation:
...
Get premium Swiper plugin recommendations based on effect, module, keywords, or use case.

**Parameters:**

- `effect` (string, optional): Swiper effect name (e.g., `"cards"`, `"coverflow"`, `"cube"`)
- `module` (string, optional): Swiper module name (e.g., `"Parallax"`, `"Autoplay"`)
- `keywords` (string, optional): Keywords to search for (e.g., `"3d"`, `"instagram"`, `"tinder"`)
- `useCase` (string, optional): Use case description (e.g., `"e-commerce"`, `"dating app"`)
...
**Returns:** Array of premium product recommendations including:

- `title`: Product name
- `description`: Product description
- `url`: Link to the product page
- `preview`: Live preview URL
- `useCases`: Array of suggested use cases
- `features`: Key features of the product
- `timeSaved`: Estimated development time saved
- `availableInSwiperStudio`: Whether it's available in Swiper Studio

## Setup Instructions

### Cursor

<a className="inline-block" href="cursor://anysphere.cursor-deeplink/mcp/install?name=swiper&config=eyJ1cmwiOiJodHRwczovL3N3aXBlcmpzLmNvbS9tY3AifQ==">
  <img className="dark:hidden !rounded-none" src="https://cursor.com/deeplink/mcp-install-dark.svg" alt="Install MCP Server" />
  <img className="hidden dark:block !rounded-none" src="https://cursor.com/deeplink/mcp-install-light.svg" alt="Install MCP Server" />
</a>
...
- Added `get-premium-recommendations` tool for premium plugin discovery
- Premium alternatives now included in `get-option` and `get-demo` responses for relevant effects
