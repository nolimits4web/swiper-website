...
<div className="important-note">
  Note that Swiper Angular component all events emits outside of NgZone for better perfomance.
  Dont forget to use `ngzone.run` or ChagngeDetector if you need to change view (e.g slides) in event handlers (e.g `slideChange`).
</div>
```html
